...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Locale-aware UI strings (English / 简体中文), picked once from the
    // browser language. Falls back to English for everything else.
    var MESSAGES = {
      en: {
        up: 'Connected to the dsh web server',
        warn: 'Connecting to the dsh web server…',
        down: 'Disconnected — the dsh web server is not answering',
        lastCheck: ' · last check ',
      },
      zh: {
        up: '已连接到 dsh web 服务器',
        warn: '正在连接 dsh web 服务器…',
        down: '已断开 — dsh web 服务器无响应',
        lastCheck: ' · 上次检查 ',
      },
    }
    ...
    function locale() {
      ...
      } catch (e) { /* navigator unavailable (tests) — English */ }
      return 'en'
    }

    function messages() { return MESSAGES[locale()] || MESSAGES.en }

    function tooltipText() {
      var m = messages()
      var l = level()
      var label = m[l]
      var checked = state.lastCheck ? m.lastCheck + state.lastCheck.toLocaleTimeString() : ''
    ...
    exports.apply = apply
    exports.__connDot = { computeLevel: computeLevel, PROBE_INTERVAL_MS: PROBE_INTERVAL_MS, messages: messages, locale: locale }
